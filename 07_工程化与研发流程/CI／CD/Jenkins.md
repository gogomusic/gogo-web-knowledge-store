...
[Jenkins](https://www.jenkins.io/zh/) 是一个独立的开源软件项目，是基于 [Java](https://www.w3cschool.cn/java/) 开发的一种持续集成工具，用于监控持续重复的工作，旨在提供一个开放易用的软件平台，使软件的持续集成变成可能。前身是 Hudson 是一个可扩展的持续集成引擎。可用于自动化各种任务，如构建，测试和部署软件。Jenkins 可以通过本机系统包 [Docker安装](https://www.w3cschool.cn/docker/)，甚至可以通过安装 Java Runtime Environment 的任何机器独立运行。
...
打开 Gitlab,进入项目，点击设置 -Webhooks，输入 [上面第5步](Jenkins.md#^8by6c8) 的 hook 地址，选择触发来源为**推送事件**，通配符样式设置为 dev。标识只推送 dev 分支的 **push** 事件。最后点击添加 webhook 即可
