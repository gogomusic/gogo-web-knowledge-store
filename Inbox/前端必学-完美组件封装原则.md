---
title: "前端必学-完美组件封装原则"
source: "https://juejin.cn/post/7536467524998103092"
author:
  - "[[Freedom风间]]"
published: 2025-08-09
created: 2026-04-13
description: "此文总结了我多年组件封装经验，以及拜读 antd、element-plus、vant、fusion等多个知名组件库所提炼的完美组件封装的经验；是一个开发者在封装项目组件，公共组件等场景时非常有必要遵循此文总结了我多年组件封装经验，以及拜读 antd、element-plus、vant、fusion等多个知名组件库所提炼"
tags:
  - "clippings"
---
...
此文总结了我多年组件封装经验，以及拜读 `antd`、`element-plus`、`vant`、`fusion` 等多个知名组件库所提炼的完美组件封装的经验；是一个开发者在封装项目组件，公共组件等场景时非常有必要遵循的一些原则，希望和大家一起探讨，也希望世界上少一些半吊子组件😄
...
- 原则上所有的 `props` 和 `ref属性` 类型都需要有注释
- 且所有属性（`props` 和 `ref属性`）禁用 `// 注释内容` 语法注释，因为此注释不会被 ts 识别，也就是鼠标悬浮的时候不会出现对应注释文案
...
```ts
ts 体验AI代码助手 代码解读复制代码interface MyInputsProps {
  // 自定义class
  className?: string
}
...
![image.png](assets/6c390a204b3a429bc64956cd924cb1a5_MD5.webp)
...
- 组件导出 `funtion` 必须有名称
- 组件 `funtion` 一般 `export default` 默认导出
...
```ts
ts 体验AI代码助手 代码解读复制代码export * from './input'
...
```ts
ts 体验AI代码助手 代码解读复制代码type DialogProps = ComponentProps<typeof Dialog> 
...
```ts
ts 体验AI代码助手 代码解读复制代码interface InputProps {
  status: string
}
```
...
## 5\. class 和 style 定义规则

- 公共组件禁用 CSS module 因为此类写法会让使用者无法修改组件内部样式；vue 的话可以用 scoped 标签来防止样式重复 也可以实现父亲可修改组件内部样式。
...
```
css 体验AI代码助手 代码解读复制代码.my-input-box {
  height: 100px;
...
任何书写的组件在有可能绑定 `ref` 情况下都需要暴露有 `ref` 属性（比如表单组件），不然开发者使用 `ref` 挂载或使用调度方法（例如：useForm）就会导致控制台报错警告。

- 原创组件：useImperativeHandle 或 直接 ref 绑定组件根节点
```tsx
...
export default ChcInput
```
- 二次封装组件：则直接 ref 绑定在原基础组件上 或 组件根节点
```tsx
import { Input } from '某组件库'
...
在组件封装时，遇到组件内部会用一些固定逻辑来渲染 UI 或者计算时，最好预留一个使用者可以随意自定义的入口，而不是只能死板采用组件内部逻辑，这样可以
...
同理复杂的 ui 渲染也可以采用用户自定义传入 `render` 方法的方式进行扩展
...
组件内部如果包含了业务内容，就会大大失去他的通用性，增加开发者心智负担。
...
const Home = (props: MyTableProps, ref: React.LegacyRef<Table>) => {
...
### 13.2. 业务组件尽可能的在内部实现业务，降低使用者的使用负担
...
但是有一个业务是当数据的 `type=1` 时，data 的值要乘 2 展示，则上面的组件使用者只能这样使用：
...
正确做法应该是将 光源和声音相似的 UI 抽离成公共组件如下例如 `MyPopup`，然后分别封装光源 `Light` 和声音 `Sound` 业务组件依赖此公共组件 `MyPopup`
...
当然也可以自己开发一个公共的 Context 组件做全局的配置继承；比如多语言、是否开启震动反馈、全局主题等等
